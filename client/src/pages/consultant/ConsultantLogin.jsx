...
const ConsultantLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/consultant/login`, {
        method: 'POST',
      ...
      >
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-white p-8">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-extrabold text-[#0e3d79] tracking-tight mb-2">Consultant Portal</h1>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Link to="/consultant/forgot-password" className="text-xs font-bold text-[#0e3d79] hover:text-[#1553a1] transition-colors">Forgot password?</Link>
              </div>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
